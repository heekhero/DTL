...
def mark_trainable_parameters(model: nn.Module, model_type):
    ...
    if model_type == 'swin_base_patch4_window7_224_in22k':
...
parser.add_argument('--data_dir', default='/mnt/ramdisk1/fumh/datasets/VTAB/vtab-1k', type=str, help='data dir')

parser.add_argument('--dataset', default='cifar_100', type=str, choices=['cifar_100', 'dtd', 'patch_camelyon', 'eurosat', 'kitti', 'dmlab', 'caltech101', 'clevr_count', 'clevr_dist', 'diabetic_retinopathy', 'dsprites_loc', 'dsprites_ori', 'pets', 'flowers102', 'resisc45', 'smallnorb_azi', 'smallnorb_ele', 'sun397', 'svhn'])
parser.add_argument('--model', default='swin_base_patch4_window7_224_in22k', type=str)
...
parser.add_argument('--r', type=int, default=4, help='r for low-rank feature transformations')
parser.add_argument('--beta', type=float, default=100.0, help='hyper-params in CSN')
parser.add_argument('--lora_before_blocks', type=str, default='0-23', help='the beginning block to extract task-specific information in CSN')
parser.add_argument('--add_after_blocks', type=str, default='14-23', help='the beginning block of side feature adaptation')
...
def main():
    ...
    if args.model == 'swin_base_patch4_window7_224_in22k':
        ...
        model = create_model(args.model, num_classes=args.num_classes, pretrained=True, drop_path_rate=args.drop_path, r=args.r, fusion_size=args.fusion_size, lora_before_blocks=args.lora_before_blocks, add_after_blocks=args.add_after_blocks, beta=args.beta, log_file=args.log_file)
    else:
        raise NotImplementedError
